...
it('drives a custom axis through the whole pipeline from one registration', function () {
    ...
    expect($items)->toHaveCount(1)
        ...
        ->and($items[0]['sample']['objects'])->toHaveCount(2); // the collapsed dimension, listed
});
...
it('inserts custom axes at an explicit priority with before:', function () {
    ...
    expect(array_keys(Storyfeed::registeredAxes()))
        ->toBe(['scene', 'actors', 'targets', 'object', 'repeat', 'composite', 'batch', 'summary.hour', 'summary.day', 'summary.week', 'summary.month']);
