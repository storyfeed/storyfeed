...
it('snapshots the scoped actor synchronously', function () {
    $activity = Storyfeed::actor('System', fn () => Storyfeed::activity('ping')->publish());
...
it('returns whatever the callback returns', function () {
    $result = Storyfeed::actor('System', fn () => 'done');
...
it('accepts a model as well as a name', function () {
    ...
    $activity = Storyfeed::actor($user, fn () => Storyfeed::activity('ping')->publish());
...
it('restores the previous resolver even when the callback throws', function () {
    ...
    try {
        Storyfeed::actor('System', fn () => throw new RuntimeException('boom'));
...
it('nests, restoring the outer scope', function () {
    Storyfeed::actor('Outer', function () {
        Storyfeed::actor('Inner', fn () => Storyfeed::activity('ping')->publish());
...
it('lets an explicit actor win inside the scope', function () {
    ...
    $activity = Storyfeed::actor('System', fn () => Storyfeed::activity('ping')->actor($user)->publish());

    expect($activity->actor_type)->toBe('user');
});

it('seeds a builder when called without a callback', function () {
    $activity = Storyfeed::actor('System')->verb('sync')->object(Delivery::create(['tracking_number' => 'A']))->publish();
